...
class BasePlatform(Entity):
    ...
    async def _async_component_changed(self, event):
        ...
        _LOGGER.debug( '### async_component_changed: slave:%d, %s', self._slave, str(self._state_constraint) )
...
class BaseStructPlatform(BasePlatform, RestoreEntity):
    ...
    def __init__(self, hub: ModbusHub, config: dict) -> None:
        ...
        self._structure: str = config.get(CONF_STRUCTURE, DEFAULT_STRUCTURE_PREFIX)
    ...
    def unpack_structure_result(self, registers: list[int]) -> str | None:
        ...
        try:
            ...
            # _LOGGER.debug( 'RORRE %s, %s', str(self._structure), byte_string)
...
class BaseSwitch(BasePlatform, ToggleEntity, RestoreEntity):
    ...
    async def async_turn(self, command: int) -> None:
        ...
        _LOGGER.debug( '### async_turn switch slave:%d, address:%d, write_type:%s, command:%s,', 
             self._slave, self._address, str(self._write_type), str(command) )
        result = await self._hub.async_pb_call(
            self._slave, self._address, command, self._write_type
        )
        # _LOGGER.debug( '### async_turn switch slave:%d, address:%d, write_type:%s, command:%s, result :: %s', 
        #      self._slave, self._address, str(self._write_type), str(command), str(result) )
        if result is None:
            self._attr_available = False
            self.async_write_ha_state()
            return
        ...
        if self._verify_delay:
            async_call_later(self.hass, self._verify_delay, self._async_update)
        else:
            await self._async_update()
    ...
    async def _async_update(self, now: datetime | None = None) -> None:
        """Update the entity state."""
        address = self._verify_address
        channel = -1
        if self._attr_board == Board.WAVESHARE_RTU_RELAY:
        ...
        # do not allow multiple active calls to the same platformasy
        if self._call_active:
            return
        self._call_active = True
        result = await self._hub.async_pb_call(
            self._slave, address, 1, self._verify_type
        )
        self._call_active = False
        if result is None:
        ...
        if self._verify_type in (CALL_TYPE_COIL, CALL_TYPE_DISCRETE):
            ...
            if self._attr_board == Board.WAVESHARE_RTU_RELAY:
                self._attr_is_on = bool(result.bits[channel] & 1)
        else:
