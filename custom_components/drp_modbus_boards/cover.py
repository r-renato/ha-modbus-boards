...
async def async_setup_platform(
    ...
    for cover in discovery_info[CONF_COVERS]:
        _LOGGER.debug( 'async_setup_platform :: cover:' + str( cover ) )
        for int_cover in cover.get(CONF_COVERS, None):
            _LOGGER.debug( 'async_setup_platform :: add cover: %s %s', str(int_cover), str(cover) )
            hub: ModbusHub = get_hub(hass, discovery_info[CONF_NAME])
            covers.append(ModbusCover(hub, cover, int_cover))
...
class ModbusCover(BasePlatform, CoverEntity, RestoreEntity):
    ...
    def __init__(
        ...
        self._attr_cover_service = internal_cover.get(CONF_NAME)
        self._register = BOARD_SENSORS[ self._attr_board ][ self._attr_cover_service ]
        ...
        self._write_type = self._input_type
        self._write_address = self._address
    ...
    async def async_update(self, now: datetime | None = None) -> None:
        ...
        _LOGGER.debug( '### Number async_update slave:%d, address:%d, write_type:%s', 
             self._slave, self._address, str(self._input_type) )
